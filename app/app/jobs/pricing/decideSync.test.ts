...
describe("first-ever price", () => {
  it("always needs approval, whatever the tolerance", () => {
    ...
    for (const toleranceBps of [0, 50, 100_000]) {
      const result = decideSync({ newBankPaymentPrice: usd("136833"), lastSyncedBankPaymentPrice: null, toleranceBps });
  ...
  it("treats undefined the same as null", () => {
    expect(decideSync({ newBankPaymentPrice: usd("1000"), toleranceBps: 50 }).decision).toBe("needs_approval");
  });
});
...
describe("unchanged price", () => {
  it("auto-applies as a no-op so the queue stays empty", () => {
    const result = decideSync({
      newBankPaymentPrice: usd("136833"),
      lastSyncedBankPaymentPrice: usd("136833"),
      toleranceBps: 50,
    });
  ...
  it("is unchanged even at a zero tolerance", () => {
    const result = decideSync({
      newBankPaymentPrice: usd("500"),
      lastSyncedBankPaymentPrice: usd("500"),
      toleranceBps: 0,
    });
    expect(result.unchanged).toBe(true);
    expect(result.decision).toBe("auto_apply");
...
describe("the tolerance boundary is inclusive", () => {
  ...
  it("auto-applies exactly AT the tolerance", () => {
    const result = decideSync({
      newBankPaymentPrice: usd("100500"),
      lastSyncedBankPaymentPrice: usd("100000"),
      toleranceBps: 50,
    });
    expect(result.decision).toBe("auto_apply");
  ...
  it("requires approval one basis point over", () => {
    const result = decideSync({
      newBankPaymentPrice: usd("100510"),
      lastSyncedBankPaymentPrice: usd("100000"),
      toleranceBps: 50,
    });
    expect(result.decision).toBe("needs_approval");
  ...
  it("auto-applies just under the tolerance", () => {
    const result = decideSync({
      newBankPaymentPrice: usd("100490"),
      lastSyncedBankPaymentPrice: usd("100000"),
      toleranceBps: 50,
    });
    expect(result.decision).toBe("auto_apply");
...
describe("increases and decreases are symmetric", () => {
  it("treats -51 bps exactly like +51 bps", () => {
    const up = decideSync({ newBankPaymentPrice: usd("100510"), lastSyncedBankPaymentPrice: usd("100000"), toleranceBps: 50 });
    const down = decideSync({ newBankPaymentPrice: usd("99490"), lastSyncedBankPaymentPrice: usd("100000"), toleranceBps: 50 });
...
describe("degenerate prior price", () => {
  it("requires approval when the prior price was zero", () => {
    ...
    const result = decideSync({
      newBankPaymentPrice: usd("100000"),
      lastSyncedBankPaymentPrice: usd("0"),
      toleranceBps: 50,
    });
    expect(result.decision).toBe("needs_approval");
    expect(result.deltaBps).toBeNull();
...
describe("the comparison is exact, not floating point", () => {
  it("does not misclassify a boundary case on a price with awkward arithmetic", () => {
    ...
    const result = decideSync({
      newBankPaymentPrice: usd("334999"),
      lastSyncedBankPaymentPrice: usd("333333"),
      toleranceBps: 50,
    });
    expect(result.decision).toBe("auto_apply");
  });
...
describe("a null tolerance disables automatic publication", () => {
  it("routes an ordinary change to manual approval", () => {
    const result = decideSync({
      newBankPaymentPrice: usd("35900"),
      lastSyncedBankPaymentPrice: usd("34900"),
      toleranceBps: null,
    });
    expect(result.decision).toBe("needs_approval");
