...
43. `CARD_UPLIFT_CEIL_WHOLE_DOLLAR_V1` is unchanged: one fixed profile rate, whole-dollar ceiling. Verified by exact-value assertions against the pre-amendment implementation, by a test showing the two rules diverge when handed the same absurd configured rate, and — more convincingly than either — by the three dev campaigns frozen before the amendment (`dev-gb-1`, `dev-gb-2`, `dev-gb-3`) still reproducing their original card prices from live frozen data. *(Wording corrected at T11: an earlier draft of this criterion claimed verification "across thousands of values and several rates". That describes neither rule's test. The tiered rule's sweep is ~154 values at `regularCardPrice.test.ts:165`; the legacy rule's coverage is the exact values and the divergence test named above. The guarantee holds as stated here — the overstated phrasing did not.)*
...
**FOLDED IN 2026-09-19 (T11 condition C1). `docs/specs/SLICE-0-FINDINGS.md` §"Contracts created by slice 1" is now the authoritative copy** — that is where a later slice's spec author is directed. The text below is retained for the record; if the two ever disagree, the register wins.
...
- **C-S2 (gate: slice 2).** Slice 2 must implement `ShopifyPriceSyncPort` over `productVariantsBulkUpdate`, must honour the `lastSyncedPriceCalculationId` compare-and-set, and must not reintroduce a JS `number` price anywhere between `Money` and the GraphQL variable.
  ...
  **(a) Derive the card price from the calculation's OWN pricing profile, never today's active profile.** The card price is deliberately not stored on `price_calculation` — there is exactly one card figure and it is a pure function of the stored bank price plus the rule that governed that calculation. Slice 2 must therefore compute `deriveRegularCardPrice(row.bankPaymentPriceMinorUnits, row.pricingProfile.fixedCardUpliftRate, row.pricingProfile.regularCardPriceRuleId)`. Reading the rule id or the rate from the currently active profile instead would re-price a historical calculation under a rule that did not exist when it was made — the same failure C-S3 forbids for campaigns, on the Buy Now path.
  **(b) The bank-basis tolerance can publish an INVERTED card-price move near a tier boundary.** The uplift schedule in `docs/BANK-CARD-PRICING.md` §3 is deliberately non-monotonic at its boundaries, so a tiny rise in the bank price can produce a fall in the published card price. Worked case: bank `$4,999.99 → $5,000.00` is **+0.0002%**, far inside the 200 bps tolerance, so it auto-applies with no human review — while the published Regular/Card Price falls **`$5,175 → $5,150`**, a 0.48% cut nobody approved. The effect is bounded (it cannot exceed the gap between two adjacent tier rates, ~0.5% of the price) and it can only occur within one tier-width of a boundary, so this is a known small leak rather than an open-ended one. Slice 2 must choose explicitly: either measure the published card delta against `toleranceBps` as well as the bank delta and route the larger of the two, or record an owner-accepted decision that sub-1% inverted moves at tier boundaries may auto-apply. Silently inheriting the bank-only measurement is not one of the options, because the figure the customer sees is the card price.
