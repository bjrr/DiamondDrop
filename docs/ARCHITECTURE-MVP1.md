...
| D9-revised | **REVISED 2026-09-17 by the owner, inverting the original decision.** The calculated price is now the **LIST price, which is the CARD price**: it carries card processing and it is what gets published. Cash-equivalent customers (ACH, wire, Zelle, cheque) pay **5% less**, to encourage cash payment and because they do not incur that fee. This is NOT a sign flip of the original (cash base + 5% uplift): 5% off list is a 5.26% card-to-cash spread. **The discount OVERRIDES the profit minimums** by explicit owner instruction — the floors bind the LIST price and the cash price may fall below the $100 minimum, which with MVP1 numbers happens under $303.03 of landed cost. The cash floor evaluation is recorded on every calculation for visibility but never enforced. The cash price FLOORS to a whole dollar rather than rounding, so the realised discount is never less than the advertised 5%. **This also resolves the payment-processing double-count flagged against the original D9:** the list price legitimately carries the 2.9% + $0.30 because the list price is the card price, so no separate cash-processing cost is needed | — | — |
