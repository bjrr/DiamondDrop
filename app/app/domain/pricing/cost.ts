...
export function calculateLandedCost(input: LandedCostInput): LandedCostResult {
  ...
  const { total, perComponent } = applyCostSideComponents(metal.plus(stones), ordered, stoneCount);

  const labour = sumOf(perComponent.filter((c) => LABOUR_TYPES.has(c.componentType)));
