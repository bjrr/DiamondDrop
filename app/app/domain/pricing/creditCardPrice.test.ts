...
import { UnknownCreditCardPriceRuleError, deriveCreditCardPrice, getCreditCardPriceRule } from "./creditCardPrice";
...
describe("deriveCreditCardPrice", () => {
  ...
  const RULE: CreditCardPriceRuleId = "CARD_UPLIFT_CEIL_WHOLE_DOLLAR_V1";

  it("prices the card 5% above cash", () => {
    // $400 cash -> $420 card, exact, no rounding involved.
    expect(deriveCreditCardPrice(40000n, RATE, RULE)).toBe(42000n);
  });
  ...
  it("ceils to a whole dollar rather than landing under the uplift", () => {
    ...
    expect(deriveCreditCardPrice(34900n, RATE, RULE)).toBe(36700n);
  });
  ...
  it("never prices card below the configured uplift, across a wide range", () => {
    ...
    for (let dollars = 100; dollars <= 5000; dollars += 7) {
      ...
      const card = deriveCreditCardPrice(cash, RATE, RULE);
  ...
  it("an uplift of 1/0.95 - 1 would yield exactly 5% off, if a clean 5% were ever wanted", () => {
    ...
    const card = deriveCreditCardPrice(40000n, new MoneyDecimal("0.052632"), RULE);
  ...
  it("returns the cash price unchanged at a zero uplift", () => {
    expect(deriveCreditCardPrice(40000n, new MoneyDecimal("0"), RULE)).toBe(40000n);
  });
  ...
  it("is exact on a value where float arithmetic diverges", () => {
    ...
    expect(deriveCreditCardPrice(100200n, RATE, RULE)).toBe(105300n);
  });
  ...
  it("stays exact beyond what a double represents", () => {
    ...
    const expected = new MoneyDecimal(huge.toString())
      ...
      .ceil()
      .times(100);
    expect(deriveCreditCardPrice(huge, RATE, RULE).toString()).toBe(expected.toString());
  });
  ...
  it("rejects an unregistered rule id rather than silently not applying uplift", () => {
    ...
    const unregistered = "CARD_UPLIFT_V2" as CreditCardPriceRuleId;
    expect(() => deriveCreditCardPrice(40000n, RATE, unregistered)).toThrow(UnknownCreditCardPriceRuleError);
  });
  ...
  it("keeps CARD_UPLIFT_CEIL_WHOLE_DOLLAR_V1 frozen for stored calculations", () => {
    ...
    const rule = getCreditCardPriceRule("CARD_UPLIFT_CEIL_WHOLE_DOLLAR_V1");
