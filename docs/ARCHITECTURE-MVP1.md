...
| D14-followups | **Architect follow-ups from the `ddb7641` review, non-blocking, carried into Slice 2.** (1) Convert the margin model to a registry keyed by id, as `priceEnding.ts` and `creditCardPrice.ts` already are — today a third model requires editing `engine.ts`, which is a seam violation. Leave the DB CHECK constraint as built; it is fail-closed. (2) Extract the duplicated round-then-end sequence out of `engine.ts`. (3) Decide override supersession and revocation before Slice 2 consumes `price_override` — the table is append-only and has no rows yet, so the shape is cheapest to fix now. (4) **Slice 2 entry criterion:** split "may we publish automatically at all" from "at what threshold". Today they are one column, so the day the owner supplies a tolerance, automatic publication switches on as a side effect — possibly before D1 credentials exist. Automatic publication must become a separate enablement defaulted off | — | — |
